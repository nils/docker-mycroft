...
RUN set -x \
  ...
  && (test -e /etc/locale.alias || sudo ln -s /etc/locale.alias /usr/share/locale/locale.alias) \
...
RUN set -x \
  && apt-get update \
  && apt-get -y install gcc make pkg-config automake libtool libasound2-dev wget unzip patch \
